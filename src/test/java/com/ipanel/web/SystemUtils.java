...
public class SystemUtils {
	 ...
	 * @return
	 */
    private static BaseInfomation base() {
    	try {
     ...
     * @return
     */
    private static List<CpuInfomation> cpu() {
    	try {
	        Sigar sigar = new Sigar();
	        CpuInfo cpuInfos[] = sigar.getCpuInfoList();
	        CpuPerc cpuPercs[] = sigar.getCpuPercList();
	        ArrayList<CpuInfomation> cpuInfomations = new ArrayList<CpuInfomation>();
	        for (int i = 0; i < cpuInfos.length; i++) { // 不管是单块CPU还是多CPU都适用
	        	CpuInfomation infomaiton = new CpuInfomation();
	        	infomaiton.setId(i + 1);
	        	
	            infomaiton.setMHz(cpuInfos[i].getMhz()); // 频率
	            ...
	            cpuInfomations.add(infomaiton);
	        }
	        return cpuInfomations;
    	} catch (Exception e) {
     ...
     * @return
     */
    private static MemInfomation memory() {
    	try {
	        Sigar sigar = new Sigar();
     ...
     * @return
     */
    private static List<DiskInfomation> disk() {
    	try {
	        Sigar sigar = new Sigar();
	        FileSystem fslist[] = sigar.getFileSystemList();
	        ArrayList<DiskInfomation> disks = new ArrayList<DiskInfomation>();
